...
//session is super global, handling score variable
	if(!isset($_SESSION['score'])){
		$_SESSION['score'] = 0; //missing underscore $_SESSION Bug
	}
	...
	if($_POST){
		...
		//check if last question if else then redirect accordingly
		if($number == $total){
			header("Location: final.php");
			exit();

		} else {
			header("Location: question.php?n=".$next); //bug fix missing dollar sign $next
